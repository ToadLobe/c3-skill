...
Set the current texture to a given [IWebGLTexture](https://www.construct.net/make-games/manuals/addon-sdk/reference/graphics-interfaces/iwebgltexture). The `sampling` parameter is optional and defaults to `"auto"`, which means the texture will be rendered with the `defaultSampling` mode the texture was created with. Otherwise it can be set to one of `"nearest"`, `"bilinear"` or `"trilinear"` to render the texture with a different sampling mode.
