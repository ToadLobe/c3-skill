...
## On this page
- [Adding script files](#internalH1Link0)
- [Execution of the main script](#internalH1Link1)
- [Using additional scripts](#internalH1Link2)
- [Integration with scripts in events](#internalH1Link3)
- [Errors](#internalH1Link4)
...
## Adding script files
Script files can be added in the *Scripts* folder of the [Project Bar](https://www.construct.net/en/make-games/manuals/construct-3/interface/bars/project-bar). Existing JavaScript files (.js) and TypeScript files (.ts) can also be imported using the *Import scripts* option instead.

![](https://construct-static.com/images/v1721/uploads/articleuploadobject/0/images/132044/add-script-file.png)
...
```
Copy The callback can be an `async` function, meaning the `await` keyword can be used inside it, so this is a convenient place to run any asynchronous initialization while also making use of the runtime script interface.
...
```
Copy Note that the function for the `"beforeprojectstart"` event can be async, and Construct will wait for it to complete before the project starts running. This means you can also use `await` in case you need to do any asynchronous initialization there.
