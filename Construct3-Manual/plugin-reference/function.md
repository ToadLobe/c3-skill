...
## On this page
- [About functions](#internalH1Link0)
- [Parameters](#internalH1Link1)
- [Advanced function features](#internalH1Link2)
- [Returning values from functions](#internalH1Link3)
- [JavaScript integration](#internalH1Link4)
- [Function conditions](#internalH1Link5)
- [Function actions](#internalH1Link6)
- [Function expressions](#internalH1Link7)
...
![Using functions to prevent repeating actions](https://construct-static.com/images/v1721/uploads/articleuploadobject/0/images/994/functions3.png)
...
```
Copy (The name still refers to C2 for legacy reasons.) Note if the Function object is not included in a project, the `c2_callFunction` function will not exist, so the if check is necessary before using it. The function with the given "name" is triggered synchronously. Parameters are optional and can be omitted, but must be provided as an array in the second argument, and parameters may only be string or number values (any other types will return as 0 in Construct). The `c2_callFunction` method also returns the return value set in Construct (if any), and also can only return a string or number.
