...
## On this page
- [Loading a JSON file](#internalH1Link0)
- [JSON paths](#internalH1Link1)
- [JSON conditions](#internalH1Link2)
- [JSON actions](#internalH1Link3)
- [JSON expressions](#internalH1Link4)
...
```
Copy The path `foo.bar` refers to the inner "bar" key, which will return the value 42.
...
```
Copy Like most of Construct JSON arrays use zero-based indices, so the path `array.0` refers to the first element (123) and `array.1` refers to the second element (456), and so on.
