...
# Layout view editing
When edit mode is on, the [Layout View](https://www.construct.net/en/make-games/manuals/construct-3/interface/layout-view) will show handles indicating the path an [instance](https://www.construct.net/en/make-games/manuals/construct-3/project-primitives/objects/instances) will take as the [timeline](https://www.construct.net/en/make-games/manuals/construct-3/project-primitives/timelines/timeline) is played. The handles are only shown for **X** and **Y** properties, as those are the only that can really show any form of useful visual feedback. The handles can be used to update the path of the corresponding instance.
...
- **Large Blue:** These represent the keyframe **X** and **Y** properties. Can be moved to update the corresponding keyframes. If there is no [property keyframe](https://www.construct.net/en/make-games/manuals/construct-3/project-primitives/timelines/property-keyframe) for either the X or Y properties, the handle will only be able to move in one axis.
...
- **Green:** These appear when the current time marker is on top of a [master keyframe](https://www.construct.net/en/make-games/manuals/construct-3/project-primitives/timelines/master-keyframe) in the [Timeline Bar.](https://www.construct.net/en/make-games/manuals/construct-3/interface/bars/timeline-bar) If the **Set Keyframes** option is used now, all the keyframes at that position will be updated.
