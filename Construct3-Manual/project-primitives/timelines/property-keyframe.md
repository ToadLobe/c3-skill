...
Property keyframes hold the values the [timeline](timeline.md) uses when it is playing. They are at the end of the timeline hierarchy so any changes made at this level will only affect the property keyframe itself.

A [property track](property-track.md) can have many different property keyframes, one for each position in the timeline with values that must be reached when animating. Unlike [master keyframes](master-keyframe.md), property keyframes cannot be moved and must have a parent master keyframe. If a property keyframe's time is changed by either dragging it or changing the value in the [Properties Bar](../../interface/bars/properties-bar.md), a new master keyframe will be created at the new position along with new property keyframes to go with it.
...
- Make changes to the [instances](../objects/instances.md) you want to animate.
...
**Enabled**
See the section on common timeline element properties in [Timelines](../timelines.md).
