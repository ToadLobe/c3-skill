...
> **Hierarchies** are another feature that help with building composite objects (see *Setting up a hierarchy* in the [Layout View](https://www.construct.net/make-games/manuals/construct-3/interface/layout-view)). Containers generally apply to picking groups of objects in event sheets, whereas hierarchies generally apply to making objects move and rotate together. Both features can be used together as well.

It is essential to be familiar with [how events work](https://www.construct.net/make-games/manuals/construct-3/project-primitives/events/how-events-work) in order to understand how containers work.
...
- **Wrap:** also selects all instances in the container, and then also wraps the selection so they stretch and rotate as one. This means you can treat containers as if they are one object. For more information see *Selection wrapping* in the [Layout View](https://www.construct.net/make-games/manuals/construct-3/interface/layout-view).
...
It's possible to add data storage objects like [Array](https://www.construct.net/make-games/manuals/construct-3/plugin-reference/array) and [Dictionary](https://www.construct.net/make-games/manuals/construct-3/plugin-reference/dictionary) to a container with another object. Despite the fact these objects are invisible, a separate instance of the object is still created for each container. This allows you to have a dedicated Array or Dictionary for each instance of an object. This can be very useful as an advanced substitute for [instance variables](https://www.construct.net/make-games/manuals/construct-3/project-primitives/objects/instance-variables), such as if a very large number of variables is necessary, or if variables need to be dynamically added and removed.
