...
## On this page
- [ISDKUtils APIs](#internalH1Link0)

---
The `ISDKUtils` interface provides general APIs intended for use with the [Addon SDK](https://www.construct.net/en/make-games/manuals/addon-sdk). It is normally accessed via `runtime.sdk`.
...
These methods are equivalent to those in [ISDKInstanceBase](https://www.construct.net/make-games/manuals/construct-3/scripting/scripting-reference/addon-sdk-interfaces/isdkinstancebase), but allow specifying an arbitrary wrapper component ID. In general it is recommended to use the instance-specific methods as cases where you need to message a different wrapper extension to your own are rare.

**getObjectClassBySid(sid)**  
Returns an [IObjectClass](https://www.construct.net/make-games/manuals/construct-3/scripting/scripting-reference/object-interfaces/iobjectclass) with the given SID (Serialization ID), or null if none found. This can be used to look up the `IObjectClass` for plugin "object" properties, which pass a SID at runtime for the property value.
