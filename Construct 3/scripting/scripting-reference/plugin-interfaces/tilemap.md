...
```
Copy For example, to flip tile ID 2 horizontally, you would use bitwise OR combining the tile ID and the flag, e.g. `2 | ITilemapInstance.TILE_FLIPPED_HORIZONTAL`. Similarly you can test if the bit is set using `tile & ITilemapInstance.TILE_FLIPPED_HORIZONTAL`.
