...
> **Warning**  
> Downloading is a browser feature and depends on the browser UI. Note that mobile apps don't run in browsers (there is no address bar etc), so the download feature isn't available there. Consider using the [Share](https://www.construct.net/en/make-games/manuals/construct-3/plugin-reference/share) plugin to share the file instead.
