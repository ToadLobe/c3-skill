...
SVG files can also be drag-and-dropped in and a [SVG Picture object](https://www.construct.net/en/make-games/manuals/construct-3/plugin-reference/svg-picture) will be created for it.
...
You can connect objects together in a hierarchy - also known as a *scene graph* - in the Layout View. This works similarly to using the [Add child](https://www.construct.net/en/make-games/manuals/construct-3/plugin-reference/common-features/common-actions#internalH1Link2) hierarchy action, but set up in the editor.
...
The layout view will preview hierarchies both when previewing [timelines](https://www.construct.net/en/make-games/manuals/construct-3/project-primitives/timelines) and when previewing [behaviors](https://www.construct.net/en/make-games/manuals/construct-3/behavior-reference) that support previewing.
...
When you click a mesh point, it will also appear selected and display properties for that mesh point in the [Properties Bar](https://www.construct.net/en/make-games/manuals/construct-3/interface/bars/properties-bar). This allows precise control over the exact details of the mesh point, as well as providing informational values such as the mesh column and row. The **Z elevation** of the mesh point can also be modified in the Properties Bar, allowing for 3D mesh distortion. To learn more, see the tutorial [Using 3D features in Construct](https://www.construct.net/en/tutorials/using-3d-construct-2746).
