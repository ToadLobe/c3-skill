...
---
**Sub-events** appear indented beneath other events. They **carry on picking instances** from where the "parent" event left off. They run after the parent event's [actions](https://www.construct.net/make-games/manuals/construct-3/project-primitives/events/actions) have finished. An example is below.
...
1. Test if any *Monster* [instances](https://www.construct.net/make-games/manuals/construct-3/project-primitives/objects/instances) are overlapping *Player*. If so, the instances involved are remembered.
2. The top event's actions would run next, but it doesn't have any.
3. The second event (note it is indented) then tests if the *Player*'s **ShieldEnabled** [instance variable](https://www.construct.net/make-games/manuals/construct-3/project-primitives/objects/instance-variables) is set. If so, it subtracts 1 from the *health* of the Monster overlapping the player.
4. The third event (also indented) tests if the *Player*'s **ShieldEnabled** instance variable is not set (see [inverting conditions](https://www.construct.net/make-games/manuals/construct-3/project-primitives/events/conditions)). If so, it subtracts 1 from the health of the *Player*.
...
To add a sub-event, **right-click** the event margin and choose `Add › Add sub-event`. Alternatively choose the `Add sub-event` option from the **Add...** link on the right, or press the <kbd>S</kbd> [keyboard shortcut](https://www.construct.net/make-games/manuals/construct-3/interface/keyboard-shortcuts).
