...
When [linking a login provider](https://www.construct.net/game-services/manuals/game-services/authentication/api-end-points/login-providers/link-login-provider) to a user, if the account with the login provider is associated with another players account in your game you can call this end point to force the login provider to be linked to this player account, disconnecting it from the other account.
...
**code guid Required**
The `forceCode` provided by the originating [link poll](https://www.construct.net/game-services/manuals/game-services/authentication/api-end-points/login-providers/link-poll) end point.
