...
The Broadcast Service allows you to create channels, and then publish messages to those channels that any player regardless as to if they are [signed in](https://www.construct.net/game-services/manuals/game-services/authentication/api-end-points/players/sign-in) or not can read.  Messages can be any form of text, HTML, BBCode or just plain text.
...
It is also possible for players to rate published messages using the [rate message](https://www.construct.net/game-services/manuals/game-services/broadcasts/api-end-points/ratings/rate-message) end point.
...
When you create a [broadcast message](https://www.construct.net/game-services/manuals/game-services/broadcasts/api-objects/broadcast-message) you must specify the language you've written the message in.  This can be any of `AR`, `BG`, `CS`, `DA`, `DE`, `EL`, `EN`, `ES`, `ET`, `FI`, `FR`, `HE`, `HU`, `ID`, `IT`, `JA`, `KO`, `LT`, `LV`, `NB`, `NL`, `PL`, `PT`, `RO`, `RU`, `SK`, `SL`, `SV`, `TH`, `TR`, `UK`, `VI`, `ZH`.

When calling endpoints such as [get message](https://www.construct.net/game-services/manuals/game-services/##b.GetMessage##) you can pass in a `language` parameter which can be any of `AR`, `BG`, `CS`, `DA`, `DE`, `EL`, `EN-GB`, `EN-US`, `ES`, `ES-419`, `ET`, `FI`, `FR`, `HE`, `HU`, `ID`, `IT`, `JA`, `KO`, `LT`, `LV`, `NB`, `NL`, `PL`, `PT-BR`, `PT-PT`, `RO`, `RU`, `SK`, `SL`, `SV`, `TH`, `TR`, `UK`, `VI`, `ZH-HANS`, `ZH-HANT` and the message content will be served in this language using DeepL's translation engine.
...
The [message object](https://www.construct.net/game-services/manuals/game-services/broadcasts/api-objects/broadcast-message) has a property `isRead`, and [channel objects](https://www.construct.net/game-services/manuals/game-services/broadcasts/api-objects/broadcast-channel) have the property `anyUnreadMessages`.  When a player requests the [get message](https://www.construct.net/game-services/manuals/game-services/##b.GetMessage##) end point, this message along with all preceeding messages in this channel will be marked as read for that player.
